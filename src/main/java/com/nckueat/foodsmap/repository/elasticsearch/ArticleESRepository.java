...
interface ArticleESTagOperation {
    ...
    public SearchAfterPage<Long> findIdsByContext(String keyword, int limit,
...
class ArticleESTagOperationImpl implements ArticleESTagOperation {
    ...
    public SearchAfterPage<Long> findIdsByContext(@NonNull String keyword, int limit,
        ...
        Criteria criteria =
                new Criteria("context").matches(keyword).or(new Criteria("title").matches(keyword));

        CriteriaQuery criteriaQuery = new CriteriaQuery(criteria);
        NativeQueryBuilder builder = new NativeQueryBuilder().withQuery(criteriaQuery)
                .withFields("id").withSort(Sort.by("id").descending())
                .withSourceFilter(new FetchSourceFilter(new String[] {"id"}, null))
                .withPageable(Pageable.ofSize(limit));
